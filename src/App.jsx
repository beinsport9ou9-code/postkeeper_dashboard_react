...
function searchBlob(post) {
  return [
    ...
    post.generated_patient_post, post.personal_note, post.url
...
export default function App() {
  ...
  const stats = useMemo(() => {
    ...
    return { total: posts.length, mediaFiles, mediaPosts, important, videos, generated, topics: topics.length, accounts: accounts.length, tags: tagEntries.length }
  }, [posts, topics, accounts, tagEntries])
  ...
  const filtered = useMemo(() => {
    ...
    return rows
  }, [posts, tab, topicFilter, priorityFilter, mediaFilter, accountFilter, tagFilter, query, sortBy])

  const recentPosts = useMemo(() => [...posts].sort((a, b) => new Date(b.created_at || 0) - new Date(a.created_at || 0)).slice(0, 8), [posts])
  const studioPosts = useMemo(() => posts.filter(hasGenerated).slice(0, 5), [posts])
...
function tabSubtitle(tab) {
  const map = {
    dashboard: 'Your visual board for clinical ideas and inspiration',
    posts: 'All saved posts, organized and searchable',
    media: 'Images and videos saved from Instagram',
    ...
    <section className="pageWrap">
      <div className="heroBento">
        <div className="heroMain">
          ...
          <h3>Turn saved Instagram posts into a visual clinical idea board.</h3>
          <p>Less clutter. More color. Faster browsing. Your posts become topics, media, scripts, and references.</p>
...
function SettingsDrawer(props) {
  const {
    ...
    topics, accounts, tagEntries, onTopic, onAccount, onTag
  } = props
  ...
  return (
    <article className="postCard clickableCard" onClick={onOpen} role="button" tabIndex={0} onKeyDown={e => { if (e.key === "Enter") onOpen() }}>
      ...
      <div className="postCardBody">
        ...
        <div className="cardMeta"><span>{post.account || 'Unknown'}</span><span>{mediaCount ? `${mediaCount} media` : 'No media'}</span></div>
  ...
  return (
    <article className="compactRow clickableCard" onClick={onOpen} role="button" tabIndex={0} onKeyDown={e => { if (e.key === "Enter") onOpen() }}>
      ...
      <div className="compactMain">
        <div className="badgeRow"><span className="badge topic">{getTopic(post)}</span><span className={`badge priority ${getPriority(post.priority).toLowerCase()}`}>{getPriority(post.priority)}</span></div>
...
function PostModal({ post, onClose, onUpdate }) {
  ...
  function saveChanges() {
    onUpdate(post.id, { priority, main_topic: mainTopic, topic: mainTopic, subtopic, tags, personal_note: personalNote })
    setEditing(false)
  }
      ...
      <div className="modalCard">
        ...
        <div className="modalHeader">
          ...
          <div>
            ...
            {post.url && <a className="ghostLink" href={post.url} target="_blank" rel="noreferrer"><ExternalLink size={15} /> Instagram</a>}
          </div>
        </div>
        ...
        <div className="modalGrid">
          ...
          <Panel title="Caption">
            ...
            <div className="actionButtons">
              ...
              {post.url && <a href={post.url} target="_blank" rel="noreferrer"><ExternalLink size={15} /> Open Instagram</a>}
            </div>
          </Panel>
